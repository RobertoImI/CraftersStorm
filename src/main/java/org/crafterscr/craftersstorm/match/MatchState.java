...
public final class MatchState {
    ...
    public static final int SELF_REVIVE_TICKS = 8 * 20;
    ...
    public static final class Member {
        ...
        // PlayerRevive integration. DOWNED is intentionally not a MatchState.Status:
        // a downed player still counts as ALIVE until the real/final death happens.
        public boolean downed;
        public UUID downedBy;
        public boolean selfReviveUsed;
        public boolean selfReviveHolding;
        public int selfReviveTicks;

        // Initial aerial spawn protection. Once deploymentProtected becomes false it
        // never becomes true again during the same match.
        public boolean deploymentProtected;
        public boolean deploymentFallProtected;
        public double deploymentX;
        public double deploymentY;
        public double deploymentZ;
    ...
    public long teamCount(String team) {
        return members.values().stream().filter(m -> teamOf(m.id).equals(team)).count();
    }

    private String availableTeam() {
        for (String team : teams()) if (teamCount(team) < teamSize) return team;
        int i = 1;
        while (teams().contains("equipo" + i)) i++;
        return "equipo" + i;
    }
    ...
    public void validateStart(boolean test) {
        ...
        for (String team : teams()) if (teamCount(team) > teamSize)
            throw new IllegalStateException("El equipo " + team + " supera el límite.");
    }
    ...
    public Map<UUID, Destination> spawnAssignments(Random random) {
        ...
        List<String> groups = new ArrayList<>(teams());
        Collections.shuffle(groups, random);
        List<Destination> points = new ArrayList<>(spawns.values());
        Collections.shuffle(points, random);
        for (int i = 0; i < groups.size(); i++) {
            String group = groups.get(i);
            Destination point = points.get(i % points.size());
    ...
    public long aliveCount() {
        return members.values().stream().filter(m -> m.status == Status.ALIVE).count();
    }
    ...
    public void start(boolean test) {
        ...
        for (Member m : members.values()) {
            ...
            m.kills = 0;
            m.reconnectTicks = RECONNECT_TICKS;
            m.lastAttacker = null;
            m.lastHit = -1000;
            m.downed = false;
            m.downedBy = null;
            m.selfReviveUsed = false;
    ...
    public boolean eliminate(UUID victim, UUID killer) {
        ...
        // A previously eliminated player can still receive credit for a delayed bleed-out
        // caused by a knock registered while they were competing.
        if (killer != null && !killer.equals(victim) && !teammates(killer, victim) && members.containsKey(killer))
            members.get(killer).kills++;
